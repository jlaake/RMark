...
#' # after fitting this intial model you can use it for starting values with other model that do not require
#' # the sin link but always check to make sure the model is converging to reasonable values.
#' mod=mark(logitNor.proc,logitNor.ddl,model.parameters=list(N=list(formula=~-1+session,link="sin"),sigma=list(formula=~-1+session,link="sin"),p=list(formula=~1,link="sin")))
#' summary(mod)
#' 
NULL
